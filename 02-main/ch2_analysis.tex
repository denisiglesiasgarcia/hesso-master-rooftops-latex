...
\subsubsection{Méthodologie}
SAM 2 adopte une architecture de streaming traitant les vidéos photogramme par photogramme, équipée d'un système de mémoire pour maintenir le contexte temporel. Les composants clés incluent :

\begin{itemize}
    \item Encodeur d'image : Hiera pré-entraîné MAE, hiérarchique pour features multi-échelles
    \item Attention mémoire : L transformers conditionnant les features actuelles sur les mémoires passées
    \item Banque mémoire : File FIFO stockant N photogrammes récents et M photogrammes avec prompts, plus des "object pointers" pour l'information sémantique
    \item Décodeur de masque : Similaire à SAM, avec tête additionnelle prédisant la visibilité d'objet
\end{itemize}

L'encodeur d'image (Hiera pré-entraîné MAE) effectue un traitement unique de chaque image pour générer des représentations vectorielles non conditionnées. Le module d'attention mémoire conditionne ensuite les caractéristiques de l'image courante en fonction des mémoires des images précédentes, utilisant des blocs transformer combinant mécanismes de self-attention et cross-attention. Le décodeur de masques, reprenant l'architecture de SAM, génère plusieurs masques candidats en cas d'ambiguïté et intègre une tête de prédiction d'occlusion permettant de détecter l'absence de l'objet dans l'image.

L'encodeur mémoire produit des représentations en fusionnant le masque prédit avec les représentations vectorielles de l'image, tandis que la banque mémoire gère une file d'attente FIFO stockant les N images récentes et les M images ayant reçu des prompts. Le système maintient également des « pointeurs d'objets » sous forme de vecteurs compacts encodant l'information sémantique de haut niveau.

L'entraînement conjoint sur données d'images et de vidéos reproduit un environnement interactif en échantillonnant des séquences de 8 images avec jusqu'à 2 images recevant des prompts. La stratégie d'initialisation des prompts se répartit entre masques de vérité terrain (50 \%), clics positifs (25 \%) et boîtes englobantes (25 \%).

\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.10\textwidth]{02-main//figures//ch2/ch2_sam2_11_architecture_sam2.png}}
...
En segmentation d'images, SAM 2 surpasse SAM sur 37 datasets zero-shot tout en étant 6 fois plus rapide (130.1 FPS vs 21.7 FPS). L'entraînement sur le mélange de données image-vidéo améliore encore les performances, atteignant 61.9% de mIoU moyen contre 58.1% pour SAM.
Pour la segmentation vidéo semi-supervisée (VOS), SAM 2 établit de nouveaux états de l'art sur les benchmarks établis. Sur DAVIS 2017, le modèle atteint 90.2% en J&F, et sur YouTube-VOS 2019, 88.6%. Les performances sont particulièrement impressionnantes sur SA-V val/test (76.8%/77.0%), démontrant la capacité à "segmenter n'importe quoi" dans les vidéos.
