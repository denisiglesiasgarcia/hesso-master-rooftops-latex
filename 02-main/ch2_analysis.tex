...
\par{\acrshort{sitg} dispose d'une couche vectorielle d'emprise des toitures des bâtiments hors sol « CAD\_BATIMENTS\_HORSOL\_TOIT » \cite{SITG2023}. Cette couche provient de la numérisation 3D des bâtiments. La Figure \ref{fig:stdl_01_couche_vectorielle} représente un exemple de cette couche avec une orthophoto en fond d'image.}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{02-main//figures/stdl_01_couche_vectorielle.png}
    \caption{Image d’exemple de la couche vectorielle d’emprise au sol des bâtiments « CAD_BATIMENTS_HORSOL_TOIT »}
    \label{fig:stdl_01_couche_vectorielle}
\end{figure}
% Figure 46 à insérer ici
...
La Figure 47 représente les bâtiments choisis par \acrshort{stdl} pour la vérité terrain.

% Figure 47 à insérer ici
...
Cette méthode consiste à utiliser les données vectorielles de l'emprise des toitures, les labelliser à l'aide des données \gls{lidar} et ensuite appliquer un algorithme random forest (similaire au un arbre de décision).

\paragraph{Données}

Les données utilisées sont la couche vectorielle de l'emprise des toitures, ainsi que les données \gls{lidar}.

Comme indiqué au chapitre « 3.2.3.6 Données de nuages de points », la rugosité et l'intensité sont deux informations complémentaires fournies par les nuages de points \gls{lidar}. La rugosité mesure la variation locale de l'altitude des points et reflète la texture de la surface. Une rugosité élevée indique la présence d'obstacles. L'intensité mesure la quantité d'énergie réfléchie par la surface et dépend des propriétés de réflectance des matériaux. Elle varie selon le type de surface et peut aider à distinguer différents matériaux.

Pour constituer le dataset (vérité terrain), \acrshort{stdl} a annoté la couche vectorielle de l'emprise des toitures avec l'aide des données \gls{lidar}. Les 3 classes résultantes sont :
\begin{itemize}
...
La méthodologie se décompose en trois étapes principales : la préparation des données \gls{lidar}, la classification par seuils manuels et la classification par forêt aléatoire (random forest).

La première étape est la préparation des données \gls{lidar} :
\begin{itemize}
...
\item Un modèle numérique de terrain (MNT) est généré à partir des points \gls{lidar} pour représenter la surface du sol.
\item La rugosité de la surface est calculée à partir du MNT à une échelle de 1 m pour caractériser les variations locales de hauteur.
...
% Tableau 11 à insérer ici

La classification avec RF est plus performante que les seuils manuels, avec des taux de satisfaction augmentant de 7 et 6 points pour \acrshort{ocan} et \acrshort{ocen} respectivement.

Ci-dessous un aperçu des résultats obtenus :

% Figure 49 à insérer ici

Dans la Figure 49 on observe les différents critères par office, l'\acrshort{ocen} semble classifier plus de surfaces comme « possiblement libre » (blanc) que l'\acrshort{ocan}. Une matrice de confiance aurait permis de mieux évaluer les résultats.

\paragraph{Discussion des résultats (\acrshort{stdl})}

Dans son rapport \acrshort{stdl} fait une analyse des résultats obtenus dans lequel ils traitent les points suivants :
\begin{itemize}
...
\item Pour \acrshort{ocan} (végétalisation), une rugosité médiane élevée est tolérée.
\item Pour \acrshort{ocen} (solaire), une grande surface continue et une faible rugosité minimale sont requises.
\end{itemize}
...
Les experts de l'\acrshort{ocen} et l'\acrshort{ocan} n'ont pas apprécié les images résultantes (Figure 50) car ils s'attendaient à des polygones plus semblables aux couches vectorielles (par exemple Figure 46). \acrshort{stdl} a pourtant bien simplifié les polygones pour améliorer leur rendu visuel.

% Figure 50 à insérer ici

L'algorithme obtient un F1-score de 0.77 et un mIoU (mean Intersection over Union) de 0.38 sur l'ensemble des données de test. Le Tableau 12 ci-dessous résume les principales métriques obtenues.

% Tableau 12 à insérer ici

Les objets (Figure 51) de plus de 1 m² et situés à plus de 1 m du bord du toit sont bien détectés, avec des F1-score entre 0.82 et 0.92.

% Figure 51 à insérer ici

% Figure 52 à insérer ici

Les objets (Figure 52) qui ont leur centroïde a plus d'un mètre du bord du toit sont bien labellisés. Le F1-score est entre 0.80 et 0.85 pour ces objets. Cependant, les objets qui ont leur centroïde proche du bord (moins d'un mètre) ne sont pas bien détectés et ont 65\% de faux positif (FP), ce qui indique que la segmentation n'est pas fiable à cette distance.

% Tableau 13 à insérer ici

La plupart (Tableau 13) des classes d'objets (bouches d'aération, balcons, végétation intensive, panneaux solaires) ont des recalls supérieurs à 0.80. Cependant, les antennes et les objets bas (fenêtres, végétation extensive, pelouses) sont plus difficiles à détecter.

% Tableau 14 à insérer ici

La surface occupée (Tableau 14) totale est sous-estimée de 38\% par rapport à la vérité terrain.

Les experts (Tableau 15) sont au moins partiellement satisfaits par plus de 69\% des toits segmentés.
...
\begin{itemize}
\item True orthophotos \cite{SITG2024}
\item Couche vectorielle des toitures modifiée comme pour la segmentation \gls{lidar}
\end{itemize}

\paragraph{Méthodologie}

La Figure 53 ci-dessous représente les principales étapes utilisées pour réaliser la segmentation d'image.

% Figure 53 à insérer ici

La première étape est la préparation des orthophotos (Figure 53 « a »). Celle-ci commence par le découpage des orthophotos avec les données vectorielles des toitures (délimitation des toitures) pour ne conserver que la toiture à segmenter. La ligne bleue représente la marge de 1 mètre (zone tampon) pour faciliter la tâche de l'algorithme. Ensuite chaque toiture fera l'objet d'une tuile (une toiture par image).

La deuxième étape est la segmentation de l'image (Figure 53 « b »). Le modèle segment anything model (SAM) est utilisé via la librairie Python « segment-geospatial » \cite{QiushengWu2023} car cette libraire permet d'utiliser SAM sur des orthophotos tout en conservant les données géospatiales. SAM va segmenter les objets sur la toiture dans chaque tuile, ce qui va produire un masque par objet segmenté.

La troisième étape est la vectorisation des objets (Figure 53 « c »). Dans cette étape, les masques segmentés sont convertis en données vectorielles. Certains de ces polygones sont supprimés basé sur des critères géométriques :
\begin{itemize}
...
La quatrième étape est l'optimisation des hyperparamètres (Figure 53 « d ») de SAM et l'évaluation de ses résultats avec la vérité terrain.

La logique est que tout ce qui n'est pas segmenté à l'intérieur du périmètre de la toiture est une surface libre.

\paragraph{Résultats}

% Figure 54 à insérer ici

La Figure 54 représente un exemple de résultat de la segmentation de plusieurs toitures. On peut observer que la segmentation rencontre des difficultés avec certains éléments de toiture tel que les puits de lumière.

% Tableau 16 à insérer ici

Les métriques obtenues (Tableau 16) sur le dataset de test sont un F1-score de 0.73 et mIoU de 0.37.

% Figure 55 à insérer ici

Les petits objets (Figure 55) avec une surface de moins d'un mètre carré sont moins bien détectés (AP d'environ 0.60). Les objets de plus d'un mètre carré sont mieux détectés (AP d'environ 0.83).

% Figure 56 à insérer ici

Comme pour la segmentation \gls{lidar}, la segmentation d'image fonctionne moins bien au bord de la toiture (Figure 56). La precision est de 0.77 pour les centroïde d'objets a plus d'un mètre du bord de la toiture contre seulement 0.51 pour ceux a moins d'un mètre.

% Tableau 17 à insérer ici

Les experts sont au minimum partiellement satisfait de 86\% des résultats.
...
Les données utilisées sont le résultat des méthodologies de segmentation \gls{lidar} et celui de la segmentation d'images. Ce sont des données vectorielles.

\paragraph{Méthodologie}

Deux méthodes de combinaison des résultats ont été testées.

La première est la concaténation des objets détectés à partir des couches vectorielles produites par les segmentations \gls{lidar} et d'images.

La deuxième est la jointure spatiale des résultats de la segmentation \gls{lidar} et d'images. Dans ce cas, il faut :
\begin{itemize}
...
Les couches vectorielles obtenues n'ont pas été évaluées par les experts. Le Tableau 18 ci-dessous résume les principales métriques obtenues.

% Tableau 18 à insérer ici

En complément du Tableau 18, le Tableau 19 récapitule l'ensemble des métriques des méthodologies. La méthodologie de classification des pans de toit n'a pas de métrique, elle n'est donc pas incluse dans le tableau.
...
Les F1-score sont similaires autour de 0.77 pour les deux méthodes de combinaison.
