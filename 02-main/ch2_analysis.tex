...
L'efficacité computationnelle est remarquable face aux détecteurs end-to-end : YOLOv12-S surpasse RT-DETR-R18 avec 42\% de vitesse supplémentaire tout en utilisant seulement 36\% des calculs et 45\% des paramètres.

Les cartes d'activation révèlent une amélioration qualitative notable, avec des contours d'objets plus nets et une activation de premier plan plus précise comparé à YOLOv10/v11 (Figure \ref{fig:ch2_yolo_10_perception}), illustrant concrètement les bénéfices des mécanismes d'attention optimisés.
...
\subsubsection{Discussion et limites}
YOLOv12 démontre avec succès l'intégration efficace des mécanismes d'attention dans YOLO, remettant en question la dominance des architectures CNN. La limitation principale est la dépendance à FlashAttention, restreignant l'utilisation aux GPU récents (Turing, Ampere, Ada Lovelace, Hopper). Les études d'ablation valident l'efficacité de chaque composant proposé.
