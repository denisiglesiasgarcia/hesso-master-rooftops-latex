...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{1\textwidth}
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.10\textwidth]{02-main/figures/ch2/ch2_sam2_07_dataset_sa1b_pays2.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{02-main//figures//ch2/ch2_sam2_10_dataset_sam2.png}}
...
L'entraînement simule des sessions d'annotation interactive réelles. Le modèle reçoit des séquences de 8 images consécutives extraites de vidéos, sur lesquelles les chercheurs simulent le comportement d'un utilisateur humain. Pour chaque séquence, jusqu'à 2 images sont sélectionnées pour recevoir des instructions (prompts) de l'utilisateur simulé.

Les prompts initiaux varient selon trois modalités : dans 50\% des cas, le modèle reçoit directement le masque de segmentation correct (ground truth), dans 25\% des cas un simple clic sur l'objet à segmenter, et dans 25\% des cas une boîte englobante délimitant l'objet. Durant l'entraînement, des clics correctifs supplémentaires sont ajoutés de manière probabiliste pour simuler les corrections qu'un utilisateur ferait si la segmentation initiale était imparfaite.

L'objectif pédagogique est d'apprendre au modèle à prédire la segmentation temporelle complète (masklet) de l'objet cible sur toutes les 8 images de la séquence, en se basant uniquement sur ces quelques instructions partielles.
...
En segmentation vidéo interactive, les évaluations sur 9 datasets densément annotés révèlent que SAM 2 surpasse systématiquement les méthodes de référence combinant SAM avec des trackers vidéo (SAM+XMem++ et SAM+Cutie). Remarquablement, le modèle atteint une précision supérieure tout en nécessitant trois fois moins d'interactions utilisateur, démontrant une efficacité d'annotation considérablement améliorée.

Pour la segmentation d'objets vidéo semi-supervisée (VOS), où seule une annotation est fournie sur la première image, SAM 2 établit de nouveaux records sur 17 datasets de référence. Les gains sont particulièrement notables sur les benchmarks les plus exigeants : 76.6\% de score J\&F sur MOSE contre 71.7\% pour la meilleure méthode précédente (Cutie-base+), et 90.2\% sur DAVIS 2017 contre 88.1\% pour les approches existantes.

En segmentation d'images, SAM 2 maintient et améliore les performances de son prédécesseur sur une suite étendue de 37 datasets, incluant les 23 benchmarks originaux de SAM. Le modèle atteint 58.9\% de mIoU avec un seul clic contre 58.1\% pour SAM, tout en fonctionnant six fois plus rapidement (130.1 contre 21.7 images par seconde). L'entraînement conjoint sur données image-vidéo pousse encore ces performances à 61.9\%, illustrant les bénéfices de l'approche multimodale.

Les évaluations en généralisation zero-shot confirment la robustesse du modèle sur des domaines variés non vus durant l'entraînement : imagerie médicale par microscopie, scènes de conduite automobile, chirurgie endoscopique robotisée, et vidéos égocentriques. Cette diversité témoigne de la capacité du modèle à s'adapter à de nouveaux contextes d'application sans réentraînement spécifique.
\subsubsection{Discussion et limites}
