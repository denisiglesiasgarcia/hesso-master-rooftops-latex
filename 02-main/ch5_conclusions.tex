...
\begin{itemize}
    \item 530 images géoréférencées de \si{\numproduct{1280x1280}} pixels provenant des orthophotos 2019 de SITG
...
Les tests sur la zone de HEPIA, qui n'était pas dans les données d'entraînement, confirment la capacité de généralisation des modèles. Les résultats révèlent aussi les limites actuelles, notamment pour les toitures praticables, végétalisées ainsi que les zones très ombragées.
...
Par rapport aux travaux du Swiss Territorial Data Lab (STDL), cette approche de segmentation sémantique surpasse les méthodes testées par STDL :
\begin{itemize}
    \item IoU de 0,741 contre 0,38-0,40 pour les segmentations LiDAR et d'images de STDL
...
\begin{itemize}
    ...
    \item Intégrer des données complémentaires (LiDAR, modèles 3D) pour mieux discriminer les surfaces
