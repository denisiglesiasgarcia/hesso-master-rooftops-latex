...
La zone de HEPIA a été choisie pour tester les modèles sur un secteur qui n'a pas servi à l'entraînement.

\subsection{Analyse qualitative}
La Figure \ref{fig:quartier_hepia_original} montre les images originales de la zone HEPIA. La Figure \ref{fig:quartier_hepia_masque_toitures} présente les mêmes images avec un masque appliqué sur les toitures (couche toitures SITG). Cette étape permet au modèle de traiter uniquement les surfaces de toitures en ignorant le reste du paysage urbain.

Les zones identifiées comme toitures (Figure \ref{fig:quartier_hepia_masque_toitures}) ne sont pas toutes évidentes. En bas à gauche, une cour intérieure est identifiée comme toiture. En bas à droite, une terrasse praticable est aussi classée comme toiture, ce qui techniquement est correct car il y a bien un bâtiment dessous, mais cela complique la tâche de segmentation. L'image contient également des terrasses dans la partie supérieure gauche qui permettront d'évaluer le comportement des modèles sur ce type de surface.
...
\end{tikzpicture}
\caption{Zone HEPIA - Masque couche toitures}
\label{fig:quartier_hepia_masque_toitures}
\end{figure}

% ===========================================================
\newpage
La Figure \ref{fig:quartier_hepia_modele_unet_mambaoutsmall} présente les résultats obtenus avec le modèle UNet + MambaOutSmall sur la zone HEPIA.
...
\end{tikzpicture}
\caption{Zone HEPIA - Unet avec MambaOutSmall}
...
\newpage
La Figure \ref{fig:quartier_hepia_modele_upernet_efficientnetv2_s} montre les résultats du modèle UPerNet + EfficientNetV2-S sur la zone HEPIA.
...
\end{tikzpicture}
\caption{Zone HEPIA - UPerNet avec EfficientNetV2-S}
...
\newpage
La Figure \ref{fig:quartier_hepia_modele_segformer_mambaoutbase} montre les résultats du modèle SegFormer + MambaOutBase sur la zone HEPIA.
...
\end{tikzpicture}
\caption{Zone HEPIA - SegFormer avec MambaOutBase}
...
\newpage
La Figure \ref{fig:quartier_hepia_modele_linknet_efficientnet_b5} illustre les résultats obtenus avec LinkNet + EfficientNet-B5 sur la zone HEPIA.

La cour intérieure de HEPIA (en bas à droite) est partiellement segmentée. Le modèle semble capable de distinguer en partie les surfaces praticables des autres toitures. Toutefois, la cour intérieure située en bas à gauche est incorrectement identifiée comme surface libre.
...
\end{tikzpicture}
\caption{Zone HEPIA - LinkNet avec EfficientNet-B5}
...
\newpage
La Figure \ref{fig:quartier_hepia_modele_unetplusplus_efficientnetv2_s} montre les résultats du modèle UNet++ + EfficientNetV2-S sur la zone HEPIA.
...
\end{tikzpicture}
\caption{Zone HEPIA - UNet++ avec EfficientNetV2-S}
...
\newpage
La Figure \ref{fig:quartier_hepia_modele_segformer_regnety_080} présente les résultats du modèle SegFormer + RegNetY-080 sur la zone HEPIA.
...
\end{tikzpicture}
\caption{Zone HEPIA - SegFormer avec RegNetY-080}
\label{fig:quartier_hepia_modele_segformer_regnety_080}
\end{figure}

\paragraph{Synthèse des résultats sur la zone HEPIA}

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Modèle} & \textbf{Balcons/Terrasses} & \textbf{Toitures praticables} & \textbf{Détection obstacles} & \textbf{Gestion ombrages} \\
\hline
UNet + MambaOutSmall & + & - & + & +/- \\
\hline
...
\hline
\end{tabular}
\caption{Tableau comparatif des performances les 6 modèles sur la zone HEPIA}
...
\textbf{Légende :}
\begin{itemize}
\item \textbf{++} : Très bonne performance
\item \textbf{+} : Bonne performance
\item \textbf{+/-} : Performance mitigée
\item \textbf{-} : Performance insuffisante
\end{itemize}
...
\begin{itemize}
\item \textbf{Balcons/Terrasses} : Tous les modèles identifient correctement les balcons avec mobilier typique
\item \textbf{Toitures praticables} : Principale difficulté commune, seul LinkNet montre une amélioration partielle
\item \textbf{Détection obstacles} : LinkNet se distingue par une précision supérieure, SegFormer + RegNetY-080 est le moins performant
\item \textbf{Gestion ombrages} : SegFormer + MambaOutBase et LinkNet + EfficientNet-B5 sont les plus robustes
\end{itemize}
